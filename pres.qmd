...
- **La Encuesta Nacional de Empleo (ENE)**, a cargo del INE, es la principal fuente de información del mercado laboral del país. 
  ...
  - Identifica y caracteriza a quienes están dentro de la fuerza de trabajo (ocupados y desocupados) y aquellas personas que están fuera de la fuerza de trabajo o inactivos. 
  - Además, reporta información detallada sobre la calidad de las ocupaciones que se generan en el mercado laboral chileno. 
- En su flujo de procesamiento, analistas de la ENE utilizan la **codificación automática** para clasificar la ocupación (CIUO-08.CL) y la rama de actividad económica (CAENES) de las personas, al nivel de desagregación de 1 y 2 dígitos.
...
## [Motivación II]{.big-par}

::: {.incremental .medium-par}
- **Los modelos de machine learning se degradan con el tiempo**, ya que deben lidiar con casos extremos, bucles de retroalimentación positiva y cambios en la distribución de los datos (*data drift*).
  - **Casos extremos** (*edge cases*): Son muestras válidas pero atípicas que exponen las debilidades de un modelo en producción, llevándolo a hacer predicciones absurdas o catastróficas.
    - Los casos extremos son inevitables, pero podemos mitigar su impacto identificando valores atípicos (outliers), diseñando estrategias de contingencia (fallback) y realizando reentrenamientos frecuentes que incluyan estos casos.
...
- Solución: **monitoreo periódico y actualización de los modelos**.
  - En el caso de la ENE, mes a mes analistas nos envían las glosas de ocupación y actividad económica correspondientes a los datos que son publicados oficialmente.
  - Entonces podemos implementar el monitoreo, **comparando el nuevo dataset mensual (de producción) con el que fue usado originalmente para entrenar los modelos (de referencia)**. 
:::



## [Modelos entrenados]{.big-par}

::: {.incremental .medium-par}
- Los modelos fueron entrenados en Pytorch utilizando una arquitectura **multi-layer gated recurrent unit (GRU) RNN**. 
- Para mejorar el aprendizaje del modelo, se agregó una capa preliminar que incluye [word embeddings](https://github.com/dccuchile/spanish-word-embeddings/) pre-entrenados.
  ...
  - En este caso, los embeddings capturan la relación entre palabras en las glosas de entrenamiento, lo que beneficia al entrenamiento del modelo.
...
::: {.incremental .medium-par}
- Cambios en la distribución de la *data* pueden ocurrir debido a errores de código en el procesamiento, variaciones en el esquema de datos de entrada, **cambios en las distribuciones de características** o **cambios en las condiciones del mundo real con el tiempo**.
:::
...
![](imagenes/monitoring/data_distribution_shifts_types.png){.fragment .fade-in-then-out width="65%"}
...
::: {.incremental .medium-par}
- **Comparamos los dataset de producción y de referencia** (dataset de entrenamiento), aplicando varios tests y métricas proporcionadas por la librería open-source [Evidently](https://docs-old.evidentlyai.com/).
:::
...
![](imagenes/monitoring/evidently_github_01.png){.fragment width="100%"}
...
::: {.incremental .medium-par}
- **1) Test Suite:** creamos tests que hacen chequeos generales sobre columnas **oficio, tareas y actividad **(sólo para CAENES).
...
::: {.incremental .medium-par}
- **2) Report Text Summary:** calculamos estadísticas generales (mean, std, etc.) para las métricas **Text Length** y **Word Count** aplicadas sobre las columnas **oficio, tareas y actividad **(sólo para CAENES).
...
::: {.incremental .medium-par}
- **3) Report Text Drift:** evaluamos data drift sobre las métricas **Text Length** y **Word Count** aplicadas a las columnas **oficio, tareas y actividad** (sólo para CAENES), usando el método de **Wasserstein distance**.
...
::: {.incremental .medium-par}
- **4) Report Embedding Drift:** evaluamos data drift en los **embeddings obtenidos para el dataset de producción y referencia**, a partir de los mismos word embeddings pre-entrenados que se usaron en el entrenamiento del modelo.
  - Se usa un **clasificador binario** para distinguir entre los embeddings calculados para ambos datasets.
...
::: {.incremental .medium-par}
- **5) Report Prediction Drift:** evaluamos data drift para la columna **prediction** obtenida de las predicciones de cada modelo, usando el método de **Jensen-Shannon distance**.
...
![](imagenes/monitoring/dashboard_monitoreo.png){.fragment width="100%"}
...
::: {.incremental .medium-par}
- Hacer las gestiones para conseguir etiquetas de la data en producción, para así evaluar peformance de modelos en el tiempo. 
- Integrar al flujo el re-entrenamiento de modelos.
  - Debiese ocurrir de acuerdo a criterios que debemos definir en base a las métricas de monitoreo obtenidas. 
:::
